...
function requestDto(r){return {...r,id:r.id,request_number:`#TR-${r.public_id}`,route:`${r.start_location} → ${r.destination}`,zeit:r.from_date&&r.to_date?`${r.from_date} – ${r.to_date}`:'',gewicht:r.weight_t?`${r.weight_t} t`:'',spur:r.loading_gauge||'',badge:r.hazardous_goods?'Gefahrgut':'Offen',gefahr:r.hazardous_goods,status:r.status};}
